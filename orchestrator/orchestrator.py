...
SELECTION_RULES = {
    "destination": {"destination", "itinerary", "budget", "weather"},
    "days":        {"itinerary", "budget", "weather"},
    "interests":   {"destination", "itinerary"},
    "travelStyle": {"budget", "itinerary"},
}
...
SYNTH_SYSTEM = (
    ...
    "(destination overview, day-by-day itinerary, budget, and a live-weather & "
    "packing guide). Merge them into ONE clean Markdown trip plan that satisfies "
    "the goal and respects the constraints. Use a title, a one-paragraph intro, "
    "then sections: '## Overview', '## Day-by-day Itinerary', "
    "'## Weather & What to Pack', '## Budget'. Keep real forecast figures. Do not "
    "repeat yourself or mention that you received separate inputs."
)


async def synthesize(user_request, beliefs, intent, results) -> str:
    combined = (
...
async def plan_trip(user_request: str, emit: Optional[EmitFn] = None, *,
    ...
    final = await synthesize(user_request, beliefs, intent, results)
