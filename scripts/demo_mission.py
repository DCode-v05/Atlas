"""
scripts/demo_mission.py — the scripted acceptance run.

Runs ONE mission under all three topologies (deterministic mock, no API key) and
VERIFIES the expected agent-to-agent handoffs actually happened on the wire:
  ...
  * group ............ many turns sharing ONE meeting contextId
  * recursion ........ a report becomes a sub-manager (depth >= 2)
  * caps ............. headcount / depth stay within limits

It prints a human-readable report and exits non-zero if any check fails.
...
async def main() -> int:
    ...
    print("=" * 66)
    print(f"  ATLAS acceptance run — mission: {MISSION}")
    print("=" * 66)
    ...
    async with httpx.AsyncClient(timeout=240) as c:
        for topo in TOPOS:
            ...
            ctxs = {m.get("contextId") for m in msgs}
            meet_ctxs = {ctx for ctx in ctxs if str(ctx).startswith("meet-")}
            org = state["org"]
            compositions[topo] = sorted(v.get("role") for v in org.values())
            m = state["metrics"]
            ...
            print(_tree(org))
            print("      performatives: " +
                  "  ".join(f"{k}×{v}" for k, v in sorted(perf.items())))
            ...
            check(m["maxDepth"] >= 2, "recursion: a report became a sub-manager (depth >= 2)")
    ...
    print("\n  ── DETERMINISM " + "─" * 6)
    same = all(compositions[t] == compositions["hierarchical"] for t in TOPOS)
    check(same, "identical team hired across all topologies (fair comparison)")

    print("\n" + "=" * 66)
    if _fails:
        print(f"  FAILED {len(_fails)} check(s): " + "; ".join(_fails))
    else:
        print("  ALL CHECKS PASSED — the org communicates exactly as designed.")
    print("=" * 66)
    rt.shutdown()
