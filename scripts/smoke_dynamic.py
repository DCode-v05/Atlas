...
Runs one mission with ATLAS_RUNTIME=dynamic and checks it completes over genuinely
separate employee processes (not threads).
...
async def main() -> int:
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=200) as c:
            r = (await c.post(gw + "/api/run", json={"mission": "Build a tiny notes app",
                                                     "topology": "hierarchical"})).json()
            run_id = r["runId"]
            state = {}
            for _ in range(360):
